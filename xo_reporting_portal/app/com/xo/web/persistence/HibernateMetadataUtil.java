...
/**
 * Global implementation of MetadataUtil for Hibernate
 *
...
public class HibernateMetadataUtil implements MetadataUtil {
        ...
        public final static HibernateMetadataUtil getInstance() {
        	SessionFactory tempSessionFactory = ((org.hibernate.jpa.HibernateEntityManagerFactory)JPAUtil.em(JPAUtil.END_USER).getEntityManagerFactory()).getSessionFactory();
                HibernateMetadataUtil instance = map.get(tempSessionFactory);
                if (instance == null) {
                        instance = new HibernateMetadataUtil();
                        instance.sessionFactory = tempSessionFactory;
                        map.put(tempSessionFactory, instance);
                }
                return instance;
        }
        ...
        public Metadata get(String entityClassName) throws IllegalArgumentException, ClassNotFoundException {
        	Class<?> entityClass = Class.forName(entityClassName);
        	return this.get(entityClass);
    }

        public Metadata get(Class<?> entityClass) throws IllegalArgumentException {
                entityClass = this.getUnproxiedClass(entityClass);
